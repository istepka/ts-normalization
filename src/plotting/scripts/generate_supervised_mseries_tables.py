...
def aggregate_metric(rows, metric, prefix=""):
    ...
    if metric == "wql":
        ...
        numerator = sum(
            row[f"{stem}{metric}"] * row[f"{stem}{metric}_n"] for row in rows
        )
        denominator = sum(row[f"{stem}{metric}_n"] for row in rows)
    if denominator <= 0:
        raise ValueError(f"non-positive {metric} denominator")
    return numerator / denominator


def model_metric(results, model, condition, statistics, benchmark, frequency, metric):
    rows = []
    ...
    return aggregate_metric(rows, metric, "model")


def reference_metric(results, baselines, name, benchmark, frequency, metric):
    if name == "seasonal_naive":
        ...
        return aggregate_metric(rows, metric, "seasonal_naive")
    rows = [
        row
    ...
    return aggregate_metric(rows, metric)
...
def render_metric_table(results, baselines, grain, metric):
    rows, headers = grain_rows(grain)
    labels = {
        "frequency": f"supervised-mseries-{metric}",
        "benchmark": f"supervised-mseries-benchmark-{metric}",
        "benchmark_frequency": f"supervised-mseries-benchmark-frequency-{metric}",
    }
    descriptions = {
        "frequency": "frequency",
        "benchmark": "benchmark",
    ...
    caption = (
        f"{metric.upper()} by {descriptions[grain]} in the supervised "
        "M-series "
    ...
    column_spec = "l" * (len(headers) + 1) + "cc|cc||ccc"
    ...
    for model_index, (model, model_label) in enumerate(MODELS.items()):
        ...
        for row_index, (benchmark, frequency) in enumerate(rows):
            scl_lag = model_metric(
                results, model, "revin", "standard", benchmark, frequency, metric
            )
            ours_lag = model_metric(
                results, model, "sit", "standard", benchmark, frequency, metric
            )
            scl_causal = model_metric(
                results, model, "revin", "causal", benchmark, frequency, metric
            )
            ours_causal = model_metric(
                results, model, "sit", "causal", benchmark, frequency, metric
            )
            cells = [
            ...
            cells += [
                shown(
                    reference_metric(
                        results, baselines, name, benchmark, frequency, metric
                    )
                )
...
def main():
    ...
    outputs = {"supervised_mseries_effects.tex": render_effect_table(results)}
    for metric in ("wql", "mase"):
        outputs[f"supervised_mseries_{metric}.tex"] = render_metric_table(
            results, baselines, "frequency", metric
        )
        outputs[f"supervised_mseries_benchmark_{metric}.tex"] = render_metric_table(
            results, baselines, "benchmark", metric
        )
        outputs[f"supervised_mseries_benchmark_frequency_{metric}.tex"] = (
            render_metric_table(results, baselines, "benchmark_frequency", metric)
        )
