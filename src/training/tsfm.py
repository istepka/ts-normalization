...
project's existing convention (see src/training/loss_space.py) of one wandb run per setup,
...
# matching src/data/loss_space.py's VAL_SEED convention ("the held-out validation set is
...
def source_breakdown(
    ...
    for level in ("dataset", "domain", "frequency"):
        group = (
            inequality.group_mean_by_source
            if reducer == "mean"
            else inequality.group_median_by_source
        )
...
def run_moment(cfg: DictConfig, index: wi.WindowIndex, wandb_run) -> dict:
    ...
    for step in range(start_step + 1, cfg.train.steps + 1):
        ...
        if step % cfg.train.eval_every == 0 or step == cfg.train.steps:
            ...
            report = {
                "pooled_global_error": inequality.pooled_mean(pooled_nmse),
                "pooled_mase": inequality.pooled_mean(pooled_mase),
...
def finalize_summary(
    ...
    early = steps <= convergence.TROUGH_STEP_CUTOFF
    if early.sum() >= 2:
        summary["log_mse_auc_through_2000"] = convergence.log_mse_auc(steps, mse)
    return summary
...
def main(cfg: DictConfig) -> None:
    ...
    # (see src/scripts/build_gifteval_window_index.py)
...
from src.metrics import convergence, inequality
