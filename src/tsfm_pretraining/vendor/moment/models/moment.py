    ...
    def __init__(
        ...
        if reduction == "mean":
            ...
            raise ValueError(
                ...
                "Only 'mean' and 'concat' are supported."
            )
...
class MOMENT(nn.Module):
    ...
    def _get_head(self, task_name: str) -> nn.Module:
        if task_name != TASKS.RECONSTRUCTION:
            warnings.warn(
                "Only reconstruction head is pre-trained. Classification and "
                "forecasting heads must be fine-tuned."
            )
        if task_name == TASKS.RECONSTRUCTION:
            return PretrainHead(
                self.config.d_model,
    ...
    def _get_transformer_backbone(self, config) -> nn.Module:
        ...
        if config.getattr("randomly_initialize_backbone", False):
            ...
            logging.info(
                "Initializing randomly initialized transformer from "
    ...
    def reconstruction(
        ...
        **kwargs,
        ...
        dec_out = self.head(enc_out)  # [batch_size x n_channels x seq_len], normalized
        normalized_reconstruction = dec_out
        ...
        return TimeseriesOutputs(
            ...
            metadata={
                ...
                "revin_mean": self.normalizer.mean,
                "revin_stdev": self.normalizer.stdev,
            },
        )
