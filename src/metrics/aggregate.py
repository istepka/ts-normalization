...
from src.metrics import convergence
...
def paired_auc_effect(history_a: dict, history_b: dict) -> dict:
    ...
    for name in shared:
        ...
        cutoff = min(
            convergence.TROUGH_STEP_CUTOFF, int(steps_a.max()), int(steps_b.max())
        )
        if (steps_a <= cutoff).sum() < 2 or (steps_b <= cutoff).sum() < 2:
            skipped.append(name)  # too few in-window checkpoints to integrate an AUC
            continue
        auc_a = convergence.log_mse_auc(steps_a, mse_a, cutoff_step=cutoff)
        auc_b = convergence.log_mse_auc(steps_b, mse_b, cutoff_step=cutoff)
