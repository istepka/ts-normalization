...
def main() -> None:
    ...
    dataset_names = gc.discover_dataset_dirs(args.corpus_root)
    print(f"found {len(dataset_names)} dataset directories under {args.corpus_root}")
    ...
    dataset_infos = [
        gc.describe_dataset(args.corpus_root, name, domain_map)
    ...
    for info in univariate_infos:
        ...
        n_scanned = 0
        for record in gc.iter_series_records(args.corpus_root, info.name, info):
            ...
            n_scanned += 1
            if (
                args.max_series_per_dataset is not None
                and n_scanned >= args.max_series_per_dataset
            ):
                break
    ...
    frequency_inventory.to_csv(args.output_dir / "frequency_inventory.csv", index=False)

    missing_value_stats = pd.DataFrame(missing_rows)
    missing_value_stats.to_csv(args.output_dir / "missing_value_stats.csv", index=False)

    log_variances = np.log10(np.asarray(variances))
    hist_counts, hist_edges = np.histogram(log_variances, bins=30)
    (args.output_dir / "variance_histogram.json").write_text(
        json.dumps(
            {
    ...
    sampling_table.to_csv(args.output_dir / "sampling_table.csv", index=False)
    ...
    summary = {
        ...
        "max_series_per_dataset_cap": args.max_series_per_dataset,
