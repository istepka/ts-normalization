...
- paired_effect.json (only with --scale-pair): the per-dataset paired
...
This script only produces numbers, not figures, following this project's
...
def main() -> None:
    ...
    if args.scale_pair is not None:
        label_a, label_b = args.scale_pair.split("=", 1)
        ...
        (args.output_dir / "paired_effect.json").write_text(
