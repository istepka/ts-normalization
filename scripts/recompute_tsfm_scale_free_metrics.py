...
dataset's physical units rather than of model quality. This script rereads the
...
def main() -> None:
    ...
    for entry in args.run:
        ...
        if key not in contexts:
            ...
            contexts[key] = build_run_context(
                cfg, index, cache, args.device, args.chunk_size, args.pooled_windows
            )
        ctx = contexts[key]
        ...
        for ckpt in checkpoints:
            ev = evaluate_checkpoint(ctx, ckpt, args.device)
    ...
    table.to_csv(args.output_dir / "scale_free_metrics.csv", index=False)
    (args.output_dir / "final_per_dataset.json").write_text(
